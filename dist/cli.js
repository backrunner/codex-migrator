...
function printMigration(result, json) {
    ...
    process.stdout.write(`JSONL: scanned ${result.jsonl.scannedFiles}, matched ${result.jsonl.matchedFiles}, changed ${result.jsonl.changedFiles} files / ${result.jsonl.changedLines} lines\n`);
    for (const db of result.sqlite) {
        if (db.skipped) {
            process.stdout.write(`SQLite: skipped ${db.database} (${db.reason})\n`);
        }
        else {
            process.stdout.write(`SQLite: ${db.table} in ${db.database}: scanned ${db.scannedRows}, changed ${db.changedRows}\n`);
        }
    }
    if (result.jsonl.samples.length > 0) {
        ...
        for (const sample of result.jsonl.samples) {
            if (sample.toProvider) {
                process.stdout.write(`- ${sample.id ?? sample.file}: ${sample.fromProvider ?? "(unknown)"} -> ${sample.toProvider}\n`);
            }
            else {
                process.stdout.write(`- ${sample.id ?? sample.file}: ${sample.fromCwd ?? "(unknown)"} -> ${sample.toCwd ?? "(unknown)"}\n`);
            }
        }
    }
...
function printRestore(result, json) {
    ...
    process.stdout.write(`Codex home: ${result.codexHome}\n`);
    process.stdout.write(`Files: ${result.restoredFiles} total, ${result.sqliteFiles} SQLite, ${result.removedWalFiles} SQLite sidecars removed\n`);
...
function printDoctor(result) {
    process.stdout.write(`Codex home: ${result.codexHome}\n`);
    process.stdout.write(`platform: ${result.platform.node} (path separator: ${JSON.stringify(result.platform.pathSeparator)})\n`);
    process.stdout.write(`sqlite3: ${result.sqlite3Available ? "available" : "missing"}\n`);
    process.stdout.write(`sessions: ${result.sessionsDir.files} files (${result.sessionsDir.path})\n`);
    process.stdout.write(`archived: ${result.archivedSessionsDir.files} files (${result.archivedSessionsDir.path})\n`);
    for (const db of result.sqlite) {
        if (db.skipped) {
            process.stdout.write(`SQLite: skipped ${db.database} (${db.reason})\n`);
        }
