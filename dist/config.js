...
export function migrateConfigToml(codexHome, spec, options) {
...
export function transformConfigToml(content, spec) {
    ...
    const duplicateProjectSections = new Set();
...
function remapProjectConfigPath(projectPath, spec) {
    if (spec.mode === "provider") {
        return undefined;
    }
...
function emptyConfigResult(skipped, reason) {
    return {
        scannedFiles: 0,
        matchedSections: 0,
        changedSections: 0,
...
function unescapeTomlBasicString(value) {
    return value.replace(/\\(["\\btnfr])/g, (_, escaped) => {
        switch (escaped) {
            case "b":
                return "\b";
            case "t":
