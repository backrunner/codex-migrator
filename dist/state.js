...
export function discoverJsonStateFiles(codexHome) {
    const files = new Set();
...
export function migrateJsonState(codexHome, spec, options) {
...
export function inspectJsonState(codexHome) {
    const files = discoverJsonStateFiles(codexHome);
    const inspected = [];
    let pathKeys = 0;
    let pathValues = 0;
...
function collectJsonStateFiles(dir, codexHome, files) {
...
function isExcludedStateDir(codexHome, dir) {
    const relative = path.relative(codexHome, dir);
    const topLevel = relative.split(path.sep)[0];
    return EXCLUDED_STATE_DIRS.has(topLevel);
}
function transformJsonValue(value, spec, stats, file) {
...
function countStatePaths(value) {
    if (typeof value === "string") {
...
function remapStatePath(candidate, spec) {
    if (spec.mode === "provider") {
        return undefined;
    }
...
function remapStateKey(key, spec) {
...
function pathStartIndexes(value) {
    const starts = [];
...
function looksPathKey(value) {
    return (looksAbsolutePath(value) ||
        value.includes(":/") ||
        /:[A-Za-z]:[\\/]/.test(value));
}
function addSample(stats, file, fromCwd, toCwd) {
...
function addProjectChange(stats, spec, fromCwd, toCwd) {
    if (spec.mode === "provider") {
        return;
    }
